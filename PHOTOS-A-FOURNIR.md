...
> Il peut être remis tel quel au photographe.

## La consigne la plus importante : des photos claires
...
dehors une bonne partie de l’année. La lumière naturelle n’est pas une contrainte,
c’est le sujet.
...
Le site a des emplacements **en hauteur** (4:5) et des emplacements **en largeur**
(3:2, 4:3, 8:5, et un bandeau très large). Une photo verticale placée dans un
emplacement large n’en garde qu’une bande centrale : le sujet devient
méconnaissable et l’image paraît démesurément agrandie.

Il faut donc **les deux cadrages**. En particulier, prévoir des prises **cadrées en
largeur** de la terrasse, des courts et de la salle : ce sont les seules que le
site ne peut pas fabriquer par recadrage.

---

## Ce qu’on cherche

- **La lumière** : naturelle, de jour. Fin de matinée ou fin d’après-midi, quand le
  soleil rase. Ni le plein midi, qui creuse des ombres noires, ni le crépuscule.
  Pas de flash de face, pas d’anneau lumineux.
- **L’exposition** : claire, sans être délavée. Les blancs restent des blancs, les
  verts restent verts, la terre battue reste ocre. Aucun filtre, aucune dominante
  bleue.
...
- **Les personnes** : pas de personnel qui pose, pas de portrait. Si quelqu’un est
  reconnaissable sur une image publiée, **son accord écrit est nécessaire** — cela
  vaut aussi pour les joueurs sur les courts.
- **Le fichier** : JPEG de qualité, à pleine résolution, sans retouche lourde. Le
  site ne retouche pas les images : il les recadre.
...
## Ce qui n’est pas une photographie
...
millésime règle le problème d’un coup.

**Pour un remplacement ponctuel**, déposez le fichier neuf dans `images/` sous
exactement le même nom, millésime compris, et aux mêmes dimensions. Rien d’autre à
faire.

**Pour un nouveau jeu de photos**, il n’y a pas d’outil : le site est en HTML
ordinaire, sans étape de fabrication. Recadrez chaque photo aux dimensions du
...
pages HTML qui les appellent — un simple rechercher-remplacer de `2026-09a` par le
nouveau millésime, dans les cinq pages, suffit.

**Les textes de remplacement sont déjà écrits.** Chaque image porte, dans la page,
une courte phrase qui la décrit (l’attribut `alt`), lue par les personnes aveugles
et par Google. Ces phrases décrivent déjà le sujet attendu : si la photographie
livrée correspond au nom du fichier, il n’y a rien à changer. Si elle s’en écarte,
corrigez la phrase en même temps que l’image.
...
- **Terrasse, parking, accès de plain-pied, vente à l’emporter.** Ce sont des
  informations que Google affiche directement dans les résultats de recherche
  locale. Elles valent une photo chacune si on veut les montrer.
